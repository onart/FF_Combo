...
for umso in 'ㅏㅐㅑㅒㅓㅔㅕㅖㅣ':
    mr=font[umso].boundingBox()
    font[umso].transform(transV(mr,STD_VV))
for umso in 'ㅗㅛㅜㅠㅡ':
    mr=font[umso].boundingBox()
    font[umso].transform(transV(mr,STD_HV))
for umso in 'ㅘㅙㅚㅝㅞㅟㅢ':   # 조합하여 만듦
    mr=font[umso].boundingBox()
    font[umso].transform(transC(mr,COMPL))
...
# 확인된 문제: 굵기 보정 사항 없음, ㅘ ㅚ 등 가로+세로 모음 균형이 안 맞음, 자음과 가로모음 일부 겹침